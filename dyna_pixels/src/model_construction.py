...
CONTINUOUS_ENCODER_TYPES = set(['ae', 'vae', 'soft_vqvae'])
...
def construct_ae_model(input_dim, args, load=True):
  ...
  if args.ae_model_type in CONTINUOUS_ENCODER_TYPES:
    if args.ae_model_type in ('ae', 'vae'):
      ...
      model = AEModel(input_dim, latent_dim=args.latent_dim, encoder=encoder,
              decoder=decoder, stochastic=stochastic)
      ...
      TrainerClass = AETrainer if args.ae_model_type == 'ae' else VAETrainer
      ...
      trainer = VQVAETrainer(model, lr=args.learning_rate, log_freq=-1)
    ...
    elif args.ae_model_type == 'hard_gumbel_ae':
      model = HardGumbelAEModel(
...
def construct_trans_model(encoder, args, act_space, load=True):
  ...
  if args.trans_model_type == 'discrete':
    ...
    trans_trainer = UniversalVQTransitionTrainer(
      trans_model, encoder=encoder, lr=args.trans_learning_rate, log_freq=-1,
      log_norms=args.log_norms, loss_type=args.vq_trans_loss_type)
