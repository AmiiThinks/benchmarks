...
{
	return (mem[index>>5]>>((index&0x1F)<<1))&0x3;
}

template <>
uint64_t NBitArray<1>::Get(uint64_t index) const
{
	return (mem[index>>6]>>((index&0x3F)))&0x1;
}
...
template <>
uint64_t NBitArray<2>::SetAndTest(uint64_t index, uint64_t val)
{
	uint64_t result = ((mem[index>>5])>>(2*(index&0x1F)))&0x3;
	val = ((val&0x3)^result)<<(2*(index&0x1F));
	mem[index>>5] ^= val;
	return result;
}

template <>
bool NBitArray<2>::SetIf(uint64_t index, uint64_t val, uint64_t equalToThisValue)
{
	uint64_t result = ((mem[index>>5])>>(2*(index&0x1F)))&0x3;
