...
namespace FasterSymmetry {
	...
	:optimize(optimizeOrder)
	{
		...
		printf("--> Starting Faster Symmetry Solver [%s] --\n", __FILE__);
		printf("-- Ranking: %s --\n", r.name());
	...
	void Solver::InitMetaData()
	{
		...
		for (int32_t x = 0; x < order.size(); x++)
		{
			...
			{
				//printf(" [%d] Group %d is at memory %llu\n", x, order[x], memoryOffset/r.getMaxP2Rank());
...
//			printf("Depth %d complete. %d new. %d of %lld complete\n", depth, written, total, r.getMaxP1Rank());
	...
	void Solver::Initial()
	{
		...
		for (int64_t p1Rank = 0; p1Rank < max1; p1Rank++)
		{
			...
			//    also note that these states can't be illegal
		...
		for (CCMove *t = m; t; t = t->next)
		{
			...
			if (!groups[p1Rank].symmetryRedundant)
			{
				...
				if (data.Get(groups[p1Rank].memoryOffset+p2Rank) == kDraw)
				{
					...
					// While we should be propagating *wins* to all parents up the tree, we may reach
					// a state that is already set as a loss! This happens when a state is a loss due to
					// our enhanced rules -- moving backwards put us into a win position for the other
					// player or a loss for ourselves. Thus, we may find us wanting to write a Win when
					// the state is already set as a Loss. This isn't a problem in the main solve
					// loop, because all of the illegal states and terminal wins/losses are already
					// set, and we won't change them. It could be a problem if we don't write out
					// illegal states immediately in the initial loops that set win/loss states.
					
//					if (data.Get(groups[p1Rank].memoryOffset+p2Rank) == kLoss)
//					{
//						printf("\n");
//						r.unrank(p1Rank, p2Rank, tmp);
//						tmp.PrintASCII();
//						printf("Children:\n");
//						CCMove *c = cc.getMoves(tmp);
...
//							cc.UndoMove(tmp, n);
//						}
//						printf("We want to assign state as Win, but it is already stored as a Loss\n");
//						printf("Curr: %d; Attempt: %d\n", data.Get(groups[p1Rank].memoryOffset+p2Rank), kWin);
//						assert(data.Get(groups[p1Rank].memoryOffset+p2Rank) != kLoss);
//					}
				}
			}

			// Only flip pieces and copy to tmp
			...
			if (!groups[p1Rank].symmetryRedundant)
			{
				...
				if (data.Get(groups[p1Rank].memoryOffset+p2Rank) == kDraw)
				{
					...
					// See above on why we skip this sanity check
					// (We could, for fun [extra confidence], just check if the state is terminal here-
					//  it must be if we are correct)
//					assert(data.Get(groups[p1Rank].memoryOffset+p2Rank) != kLoss);
				}
			}
...
//				exit(0);
			...
			case kLoss:
				...
				if (data.SetIf(groups[r1].memoryOffset+r2, kLoss, kDraw))
				{
					...
					assert(data.Get(groups[r1].memoryOffset+r2) != kWin);
				}
				break;
			case kDraw: // still unknown
				break;
...
//		int64_t max1 = r.getMaxP1Rank();
	...
	{
		if (optimize)
			printf("-- Starting Faster Symmetry Solver solver (optimized ordering) --\n");
		else
			printf("-- Starting Faster Symmetry Solver solver (no order optimization) --\n");
		printf("-- Ranking: %s --\n", r.name());
		Timer t, total;
		CCheckers cc;
		uint64_t maxRank = r.getMaxRank();
	...
	void Solver::PrintStats() const
	{
		...
		for (uint64_t x = 0; x < data.Size(); x++)
		{
			switch (data.Get(x))
			{
				case FasterSymmetry::kWin: w++; break;
				case FasterSymmetry::kLoss: l++; break;
				case FasterSymmetry::kIllegal: i++; break;
				case FasterSymmetry::kDraw: d++; break;
			}
		}
		...
		for (uint64_t x = 0; x < r.getMaxP1Rank(); x++)
		{
			...
			for (uint64_t y = 0; y < r.getMaxP2Rank(); y++)
			{
				...
				{
					case FasterSymmetry::kWin: w+=(sym?2:1); break;
					case FasterSymmetry::kLoss: l+=(sym?2:1); break;
					case FasterSymmetry::kIllegal: i+=(sym?2:1); break;
					case FasterSymmetry::kDraw: d+=(sym?2:1); break;
				}
			}
		}
		uint64_t tmp = w;
