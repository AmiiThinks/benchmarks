...
class Train():
    def __init__(self, config, self_play_iter):
        ...
        self.train_set = config["file"]["training_data_parallel"]
        self.trained_model_params_path_parallel = config["file"]["trained_model_params_path_parallel"]
        self.training_plots_parallel_path = config["file"]["training_plots_parallel"]
        self.logs_path_parallel = config["file"]["logs_path_parallel"]
        self.total_time = 0
        ...
        # define logger
        self.logger = log_helper.setup_logger('parallel_train', self.logs_path_parallel, 'parallel_train.log')
    ...
    def _load_model(self):
        # init params and st from trained model if iteration > 0
        if self.self_play_iter > 0:
            self.logger.info('Loading trained model parameters from iteration: ' + str(self.self_play_iter - 1))
            params_path = os.path.join(self.trained_model_params_path_parallel, 'mod_{}'.format(self.self_play_iter - 1))
            opt_state_path = os.path.join(self.trained_model_params_path_parallel, 'opt_{}'.format(self.self_play_iter - 1))
    ...
    def train(self, buffer_q, self_play_iter):
        ...
        for epoch in range(1):
            ...
            # save model
            utils.save_model(self.trained_model_params_path_parallel+'mod_{}'.format(self.self_play_iter), self.params)
            utils.save_optimizer_state(self.trained_model_params_path_parallel+'opt_{}'.format(self_play_iter), self.opt_state)

            # save total loss for plotting
            utils.plot_loss(self.training_plots_parallel_path, self.self_play_iter, self.total_loss_arr, self.iteration_arr, name='total_loss')
            utils.plot_loss(self.training_plots_parallel_path, self.self_play_iter, self.value_loss_arr, self.iteration_arr, name='value_loss')
            utils.plot_loss(self.training_plots_parallel_path, self.self_play_iter, self.policy_loss_arr, self.iteration_arr, name='policy_loss')
            utils.plot_losses_in_single_line(self.training_plots_parallel_path) # plot all losses in a single line

            # plot losses and save to file
            utils.plot(self.training_plots_parallel_path, self.iteration_arr, self.total_loss_arr, self.value_loss_arr, self.policy_loss_arr, self.self_play_iter)
